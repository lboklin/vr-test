...
var step_size = 0.5
var original_controller_rot = null
...
func _physics_process(delta):
    ...
    var controller_on = controller and controller.get_is_active()
    # button 14 is mapped to the pad
    if controller_on and controller.is_button_pressed(14):
        if not original_controller_rot:
            original_controller_rot = controller.global_transform.basis.z.x

#        var x = controller.get_joystick_axis(0)
#        var y = controller.get_joystick_axis(1)

        # Teleport using center thumb pad press
#        if Vector2(x, y).length() > controller.pad_center_radius:
#            return
...
#            teleport_rotation += (delta * controller.get_joystick_axis(0) * -4.0)
            var current_controller_rot = controller.transform.basis.x.y
            teleport_rotation = current_controller_rot * 2.0 #- original_controller_rot
