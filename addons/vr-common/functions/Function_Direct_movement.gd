...
func _physics_process(delta):
    ...
    if controller.get_is_active() and controller.is_button_pressed(14):
        var left_right = controller.get_joystick_axis(0)
        var forwards_backwards = controller.get_joystick_axis(1)

        # We only respond to the edges of the pad
        var from_center = Vector2(left_right, forwards_backwards).length()
        if from_center < controller.pad_center_radius:
            return
